...
if __name__ == "__main__":
    ...
    config = Config()
    config.read_checkpoint(args.checkpoint, "build/config.bin")

    tokenizer = Tokenizer()
    tokenizer.build_tokenizer(args.tokenizer, config.vocab_size)
    tokenizer.save_tokenizer("build/tokenizer.bin")
    tokenizer.free_tokenizer()

    weights = Weights()
    weights.read_weights(args.checkpoint, "build/weights.psp")
